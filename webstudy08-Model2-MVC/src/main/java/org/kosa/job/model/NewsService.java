 ...
 * : 시스템 상에서 단 한번 객체를 생성해 공유해 사용하는 디자인 패턴
 * -> 스프링에서 기본적으로 객체를 운용하는 방식(managing 방식)
 * why? 서버 자원의 효율성: 불필요한 반복적으로 객체를 여러개 생성하지 않게해서 자원 절약
 * 		실제로 스프링 bean(자바컴포넌트), 서블릿이 컨테이너에 의해 싱글톤으로 매니징 된다.
 * 
 * private Constructor
...
public class NewsService {
	...
	private NewsService(){ // 외부에서 객체 생성 못하게 봉쇄
		System.out.println(getClass()+"단 한번 객체를 생성해 사용");
	}
	public static NewsService getInstance() {
		return instance;
	}
	public String findNewById(int newsId) {
